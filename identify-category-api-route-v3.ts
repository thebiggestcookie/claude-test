...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const prompt = `Given the product "${product}", the suggested category "${category.name}" in the department "${category.department.name}", and the subcategory "${subcategory.name}", confirm if these are correct or suggest more appropriate ones. Respond in JSON format with keys: "categoryConfirmed" (boolean), "subcategoryConfirmed" (boolean), "suggestedCategory" (string, if different), and "suggestedSubcategory" (string, if different).`;
    ...
    res.status(200).json({ 
      ...
      originalSubcategory: subcategory.name,
      debug: {
        prompt,
        llmResponse
      }
